...
<div class="mb-6">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route($routePrefix . '.customers.index') }}" class="px-6 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all">
            Add New Customer
        </a>
    </div>
</div>
...
<div class="bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
    <form method="GET" action="{{ route($routePrefix . '.customers.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <!-- Search -->
        <div>
        ...
        <div class="flex items-end gap-2 md:col-span-2 lg:col-span-4">
            ...
            <a href="{{ route($routePrefix . '.customers.index') }}" class="px-4 py-1.5 text-sm bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition-all">
                Clear
            </a>
        </div>
    </form>
                ...
                <tr class="hover:bg-gray-700 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <div class="flex gap-2">
                            <a href="{{ route($routePrefix . '.customers.show', $customer) }}" class="px-2 py-1 bg-indigo-600 text-white text-xs rounded hover:bg-indigo-700">
                                View
                            </a>
                            <a href="{{ route($routePrefix . '.customers.edit', $customer) }}" class="px-2 py-1 bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-xs rounded hover:from-emerald-700 hover:to-teal-700">
                                Edit
                            </a>
                            @if(($customer->orders_count ?? 0) == 0)
                            <form action="{{ route($routePrefix . '.customers.destroy', $customer) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this customer?');">
                                @csrf
                                @method('DELETE')
