...
<div class="mb-6 flex justify-between items-start">
    ...
    <a href="{{ route($routePrefix . '.processing.show', $processing) }}" 
...
<form action="{{ route($routePrefix . '.processing.update', $processing) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <button type="button" onclick="if(confirm('Are you sure you want to DELETE this processing request? This action cannot be undone.')) { document.getElementById('deleteForm').submit(); }" 
...
<!-- Delete Form -->
@if($processing->status !== 'completed')
<form id="deleteForm" action="{{ route($routePrefix . '.processing.destroy', $processing) }}" method="POST" class="hidden">
    @csrf
    @method('DELETE')
</form>
@endif
