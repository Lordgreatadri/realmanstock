...
<div class="bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
    <form method="GET" action="{{ route($routePrefix . '.orders.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <!-- Search -->
        <div>
        ...
        <div class="flex items-end gap-2">
            ...
            <a href="{{ route($routePrefix . '.orders.index') }}" class="px-4 py-1.5 text-sm bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition-all">
                Clear
            </a>
        </div>
    </form>
            ...
            <tbody class="divide-y divide-gray-700">
                ...
                <tr class="hover:bg-gray-700 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <a href="{{ route($routePrefix . '.orders.show', $order) }}" class="text-indigo-400 hover:text-indigo-300 font-medium">
                            {{ $order->order_number }}
                        </a>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <div class="flex gap-2">
                            <a href="{{ route($routePrefix . '.orders.show', $order) }}" class="px-2 py-1 bg-indigo-600 text-white text-xs rounded hover:bg-indigo-700">
                                View
                            </a>
                            <a href="{{ route($routePrefix . '.orders.edit', $order) }}" class="px-2 py-1 bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-xs rounded hover:from-emerald-700 hover:to-teal-700">
                                Edit
                            </a>
                            @if($order->status !== 'delivered')
                            <form action="{{ route($routePrefix . '.orders.destroy', $order) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this order?');">
                                @csrf
                                @method('DELETE')
