...
<div class="max-w-4xl mx-auto space-y-6">
    <!-- Page Header -->
    <div>
        <h1 class="text-3xl font-bold text-white">Edit Animal</h1>
        <p class="text-gray-400 mt-1">Update animal information</p>
    </div>

    <!-- Edit Form -->
    <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <form method="POST" action="{{ route($routePrefix . '.animals.update', $animal) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="date_of_birth" id="date_of_birth" value="{{ old('date_of_birth', $animal->date_of_birth?->format('Y-m-d')) }}"
                        class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    @error('date_of_birth')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="current_weight" id="current_weight" value="{{ old('current_weight', $animal->current_weight) }}"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="purchase_price" id="purchase_price" value="{{ old('purchase_price', $animal->purchase_price) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="purchase_date" id="purchase_date" value="{{ old('purchase_date', $animal->purchase_date?->format('Y-m-d')) }}" required
                        class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    @error('purchase_date')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="supplier" id="supplier" value="{{ old('supplier', $animal->supplier) }}"
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="available" {{ old('status', $animal->status) === 'available' ? 'selected' : '' }}>Available</option>
                        <option value="quarantined" {{ old('status', $animal->status) === 'quarantined' ? 'selected' : '' }}>Quarantined</option>
                        <option value="under_treatment" {{ old('status', $animal->status) === 'under_treatment' ? 'selected' : '' }}>Under Treatment</option>
                        <option value="reserved" {{ old('status', $animal->status) === 'reserved' ? 'selected' : '' }}>Reserved</option>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="selling_price_per_kg" id="selling_price_per_kg" value="{{ old('selling_price_per_kg', $animal->selling_price_per_kg) }}"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="fixed_selling_price" id="fixed_selling_price" value="{{ old('fixed_selling_price', $animal->fixed_selling_price) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="flex items-center">
                    <input type="checkbox" name="is_vaccinated" id="is_vaccinated" value="1" {{ old('is_vaccinated', $animal->is_vaccinated) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <input type="date" name="last_vaccination_date" id="last_vaccination_date" value="{{ old('last_vaccination_date', $animal->last_vaccination_date?->format('Y-m-d')) }}"
                        class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
                ...
                <textarea name="health_notes" id="health_notes" rows="3"
                    ...
                    class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('health_notes', $animal->health_notes) }}</textarea>
                @error('health_notes')
                    <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3"
                    ...
                    class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('notes', $animal->notes) }}</textarea>
                @error('notes')
                    <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    <div id="imagePreview" class="{{ $animal->image ? '' : 'hidden' }} w-24 h-24 rounded-lg overflow-hidden border-2 border-gray-700">
                        <img src="{{ $animal->image ? asset('storage/' . $animal->image) : '' }}" alt="Preview" class="w-full h-full object-cover">
                    </div>
                    <div class="flex-1">
