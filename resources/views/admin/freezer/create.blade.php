...
@section('title', 'Add Freezer Inventory')

@section('content')
<div class="mb-6">
    <div class="flex items-center gap-4 mb-4">
        <a href="{{ route($routePrefix . '.freezer.index') }}" class="text-gray-400 hover:text-white transition">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
            </svg>
        </a>
        <div>
            <h1 class="text-3xl font-bold text-white">Add Freezer Inventory</h1>
            <p class="text-gray-400 mt-1">Add new item to freezer inventory</p>
        </div>
    </div>
</div>

<div class="bg-gray-800 rounded-lg shadow-lg p-6">
    <form action="{{ route($routePrefix . '.freezer.store') }}" method="POST">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="processing_request_id" id="processing_request_id"
                    ...
                    @foreach($processingRequests as $request)
                        <option value="{{ $request->id }}" {{ old('processing_request_id') == $request->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" name="selling_price_per_kg" id="selling_price_per_kg" step="0.01" required
                       value="{{ old('selling_price_per_kg') }}"
            ...
            <div>
                ...
                <input type="date" name="processing_date" id="processing_date" required
                       value="{{ old('processing_date', date('Y-m-d')) }}"
            ...
            <div>
                ...
                <select name="temperature_zone" id="temperature_zone" required
                    ...
                    <option value="freezer" {{ old('temperature_zone') == 'freezer' ? 'selected' : '' }}>Freezer (-18°C or below)</option>
                    <option value="chiller" {{ old('temperature_zone') == 'chiller' ? 'selected' : '' }}>Chiller (0-4°C)</option>
                    <option value="cold_room" {{ old('temperature_zone') == 'cold_room' ? 'selected' : '' }}>Cold Room (4-8°C)</option>
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="quality_notes" id="quality_notes" rows="3"
                          class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-indigo-500 @error('quality_notes') border-red-500 @enderror">{{ old('quality_notes') }}</textarea>
                @error('quality_notes')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-6 flex gap-4">
            ...
            <a href="{{ route($routePrefix . '.freezer.index') }}" class="px-6 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition">
                Cancel
            </a>
        </div>
    </form>
