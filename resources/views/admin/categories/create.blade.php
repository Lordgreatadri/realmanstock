...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div>
        ...
        <p class="text-gray-400 mt-1">Add a new category for livestock, grocery, or service</p>
    </div>

    <!-- Create Form -->
    <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <form method="POST" action="{{ route($routePrefix . '.categories.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required
                    placeholder="e.g., Cattle, Goat, Frozen Chicken"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    ...
                    class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-gray-800">
                <a href="{{ route($routePrefix . '.categories.index') }}" class="px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition">
                    Cancel
                </a>
