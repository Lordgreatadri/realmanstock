...
<div class="mb-6">
    <div class="flex items-center gap-4 mb-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-white">Add Store Item</h1>
            <p class="text-gray-400 mt-1">Add new item to store inventory</p>
        </div>
    </div>
</div>

<div class="bg-gray-800 rounded-lg shadow-lg p-6">
    <form action="{{ route($routePrefix . '.store-items.store') }}" method="POST">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- SKU -->
            <div>
                <label for="sku" class="block text-sm font-medium text-gray-300 mb-2">SKU (auto-generated if empty)</label>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" id="notes" rows="2"
                          class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-indigo-500 @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-6 flex gap-4">
            ...
            <a href="{{ route($routePrefix . '.store-items.index') }}" class="px-6 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition">
                Cancel
            </a>
        </div>
    </form>
