...
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route($routePrefix . '.store-items.index') }}" class="text-gray-400 hover:text-white transition">
            ...
            <div>
                ...
                <p class="text-gray-400 mt-1">{{ $item->name }}</p>
            </div>
        </div>
        <div class="flex gap-2">
            <a href="{{ route($routePrefix . '.store-items.edit', $item) }}" 
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-white font-semibold font-mono">{{ $item->sku }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-white font-semibold">{{ $item->category->name }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-400">Supplier</p>
                    <p class="text-white font-semibold">{{ $item->supplier ?: 'N/A' }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-white font-semibold">{{ ucfirst($item->unit) }}</p>
                </div>
            </div>
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-3xl font-bold text-white">{{ number_format($item->quantity, 2) }} <span class="text-lg text-gray-400">{{ $item->unit }}</span></p>
                    @if($item->quantity <= 0)
                ...
                <div>
                    ...
                    <p class="text-3xl font-bold text-yellow-400">{{ number_format($item->reorder_level, 2) }} <span class="text-lg text-gray-400">{{ $item->unit }}</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-2xl font-bold text-white">GH₵{{ number_format($item->cost_price, 2) }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-400">Selling Price</p>
                    <p class="text-2xl font-bold text-indigo-400">GH₵{{ number_format($item->selling_price, 2) }}</p>
                </div>
            </div>
            ...
            <div class="mt-4 grid grid-cols-2 gap-4">
                <div class="p-4 bg-gray-700 rounded-lg">
                    ...
                    <p class="text-xl font-bold text-green-400">GH₵{{ number_format($item->selling_price - $item->cost_price, 2) }}</p>
                </div>
                <div class="p-4 bg-gray-700 rounded-lg">
            ...
            <div class="mt-4 p-4 bg-gray-700 rounded-lg">
                ...
                <p class="text-2xl font-bold text-white">GH₵{{ number_format($item->quantity * $item->cost_price, 2) }}</p>
            </div>

            <div class="mt-4 p-4 bg-gray-700 rounded-lg">
                <p class="text-sm text-gray-400">Potential Revenue (at selling price)</p>
                <p class="text-2xl font-bold text-indigo-400">GH₵{{ number_format($item->quantity * $item->selling_price, 2) }}</p>
            </div>
            @endif
        </div>
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            <p class="text-white">{{ $item->notes }}</p>
        </div>
        @endif
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            <form action="{{ route($routePrefix . '.store-items.adjust-stock', $item) }}" method="POST">
                @csrf
                <div class="space-y-3">
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            <form action="{{ route($routePrefix . '.store-items.toggle-status', $item) }}" method="POST">
                @csrf
                @method('PATCH')
                <button type="submit" class="w-full px-4 py-2 {{ $item->is_active ? 'bg-gray-600 hover:bg-gray-700' : 'bg-green-600 hover:bg-green-700' }} text-white rounded-lg transition">
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <p class="text-white text-sm">{{ $item->created_at->format('M d, Y h:i A') }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-400">Last Updated</p>
                    <p class="text-white text-sm">{{ $item->updated_at->format('M d, Y h:i A') }}</p>
                </div>
            </div>
        </div>
    </div>
