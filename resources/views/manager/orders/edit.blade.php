...
<div class="mb-6">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('manager.orders.show', $order) }}" class="px-6 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition-all">
            Cancel
        </a>
    </div>
</div>
...
<form action="{{ route('manager.orders.update', $order) }}" method="POST">
    @csrf
    @method('PUT')
...
<div class="mt-6">
    <form action="{{ route('manager.orders.destroy', $order) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this order? This action cannot be undone.');">
        @csrf
        @method('DELETE')
