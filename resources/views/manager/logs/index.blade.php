...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <div class="flex space-x-3">
            @if($selectedFile)
                <a href="{{ route('manager.logs.download', $selectedFile) }}" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition">
                ...
                <div class="space-y-2">
                    @forelse($logFiles as $file)
                        <a href="?file={{ $file['name'] }}" 
                           class="block p-3 rounded-lg transition {{ $selectedFile === $file['name'] ? 'bg-green-600 text-white' : 'bg-gray-900 text-gray-300 hover:bg-gray-700' }}">
                            <div class="flex items-center justify-between">
                                <div class="flex-1 truncate">
                                    <div class="text-sm font-medium truncate">{{ $file['name'] }}</div>
                                    <div class="text-xs {{ $selectedFile === $file['name'] ? 'text-green-200' : 'text-gray-500' }} mt-1">
            ...
            <div class="bg-gray-800 border border-gray-700 rounded-lg p-4">
                <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="level" class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-green-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="{{ $date }}" 
                               class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-green-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-2">Search</label>
                        <input type="text" name="search" value="{{ $search }}" placeholder="Search logs..." 
                               class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:ring-2 focus:ring-green-500">
                    </div>
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    @if($log['file'])
                                        <div class="mt-2 text-xs">
                                            ...
                                            <span class="text-green-400 font-mono">{{ $log['file'] }}</span>
                                            @if($log['line'])
                                                <span class="text-gray-500">:</span>
                                    ...
                                    <!-- Toggle Stack Trace -->
                                    @if($log['trace'])
                                        <button @click="expanded = !expanded" class="mt-2 text-xs text-green-400 hover:text-green-300">
